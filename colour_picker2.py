...
# AP: good practice to have these at the top of the script
...
def on_activate():
    ...
    log_message(
        f"RGB: {last_picked_formats['rgb']}, HSL: {last_picked_formats['hsl']}, CMYK: {last_picked_formats['cmyk']}"
    )
...
def copy_specific_color(hex_color, formats, format_type):
    if format_type == "hex":
        ...
        color_str = (
            f"rgb({formats['rgb'][0]}, {formats['rgb'][1]}, {formats['rgb'][2]})"
        )
...
def show_context_menu(event, hex_color, formats):
    ...
    context_menu.add_command(
        label="Copy HEX", command=lambda: copy_specific_color(hex_color, formats, "hex")
    )
    context_menu.add_command(
        label="Copy RGB", command=lambda: copy_specific_color(hex_color, formats, "rgb")
    )
    context_menu.add_command(
        label="Copy HSL", command=lambda: copy_specific_color(hex_color, formats, "hsl")
    )
    context_menu.add_command(
        label="Copy CMYK",
        command=lambda: copy_specific_color(hex_color, formats, "cmyk"),
    )
...
def save_palette():
    ...
    palette_data = {"grid_rows": grid_rows, "grid_cols": grid_cols, "colors": []}

    for hex_color, formats in color_history:
        palette_data["colors"].append({"hex": hex_color, "formats": formats})
...
def show_color_window():
    ...
    if not color_window or not color_window.winfo_exists():
        ...
        save_button = tkinter.Button(
            control_panel, text="Save Palette", command=save_palette
        )
        save_button.pack(side="left", padx=5)

        load_button = tkinter.Button(
            control_panel, text="Load Palette", command=load_palette
        )
...
def add_color_frame(hex_color, formats, index):
    ...
    frame.bind(
        "<Button-3>", lambda e, h=hex_color, f=formats: show_context_menu(e, h, f)
    )
    color_display.bind(
        "<Button-3>", lambda e, h=hex_color, f=formats: show_context_menu(e, h, f)
    )
